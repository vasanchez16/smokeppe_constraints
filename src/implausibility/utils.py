...
def calculate_implausibility(nc_file_path, stats_dist_method, variance_mle, nu, epsilon):
    ...
    # Determine the axis dynamically
    if my_distances.ndim == 4:
        axis = (0, 1, 2)
    elif my_distances.ndim == 2:
        axis = (0,)
    else:
        raise ValueError("Unexpected number of dimensions in my_distances")

    implaus = np.sqrt(np.nansum((my_distances ** 2) / my_variances_adjusted, axis = axis))

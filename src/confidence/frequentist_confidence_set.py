...
def frequentist_confidence_set(args):
    """
    Compute the frequentist confidence set and produce implausibility plots.

    Reads the implausibility values and MLE statistics, derives the implausibility
    threshold for the configured method and confidence level, normalises both the
    implausibilities and threshold by sqrt(num_points), then generates 1-D and
    all-parameter scatter plots coloured by plausibility.

    The normalised implausibilities and threshold are saved to:
    - norm_implausibilities.csv
    - implausibilityThreshold.csv
    """
    print('---------FreqConfSet---------')
    ...
    obs_df = pd.read_csv(eval_params['satellite_file_path'])
